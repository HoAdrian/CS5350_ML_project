...
def cross_validation(data, num_folds=5):
    ...
    # regs = [0.2, 1, 10, 100, 1000]
    learn_rates = [0.1, 0.2, 0.001, 0.01]
    regs = [5000, 1]
    ...
    for reg in regs:
        for lr in learn_rates:
            ...
            for valid_fold_idx in range(num_folds):
                ...
                logistic.train(train_data_combine, dev_data=train_data, reg=reg, lr=lr, num_epochs=10, lr_decay=True, verbose=False)
                valid_fold_data = folds[valid_fold_idx]
                accuracy = logistic.accuracy(valid_fold_data)
    ...
    for reg in regs:
        for lr in learn_rates:
            ...
            print(f"regularization sigma^2: {reg}\tlearning rate: {lr}\tmean accuracy: {avg_accuracy}")
    ...
    print(f"best regularization sigma^2 = {best_reg}")
...
if __name__ == "__main__":
    ...
    best_reg, best_lr = cross_validation(train_data, num_folds=5)